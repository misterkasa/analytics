...
str(Groceries) #not in data frame format
Groceries
arules::LIST(Groceries[1:6])  #another view to see first 6 transactions and which items were purchased, not considering the quantities
arules::inspect(Groceries[1:5]) #another view to check list

#Find Frequent Itemset
frequentItems = eclat (Groceries, parameter = list(supp = 0.01, minlen= 2, maxlen = 5)) #does not tell about confidence levels
inspect(frequentItems[1:10]) #not more than 10 else system will hang
frequentItems
...
itemFrequencyPlot(Groceries,topN = 15,type="absolute") #top 15 items displayed
itemFrequencyPlot(Groceries, topN = 10, type='relative') #relative to number of transactions
abline(h=0.15) #show which has sold more than 15% 
...
subset1 = subset(rules2, subset=rhs %in% 'whole milk' ) #rules of subset on Rules 2
inspect(subset1)
